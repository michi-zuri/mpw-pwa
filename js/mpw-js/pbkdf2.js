...
window.pbkdf2 = function() {
    function pbkdf2_js(password, salt, iter, keyLen, hash) {
        ...
        let data = new Uint8Array(salt.length + 4);
        ...
        return window.crypto.subtle.importKey("raw", password, {
            ...
            for (let block = 1, dki = 0; block <= numBlocks; block++, dki += hashLen) {
                x = x.then(() => dataView.setUint32(salt.length, block, false)).then(() => window.crypto.subtle.sign({
                    name: "HMAC",
                    hash: hash
                }, key, data)).then(pdk => (dk.set(new Uint8Array(pdk), dki), pdk));
    ...
    if (window.crypto.subtle) {
        return function(password, salt, iter, keyLen, hash) {
            ...
            return window.crypto.subtle.importKey("raw", password, {
                name: "PBKDF2",
                hash: hash
            }, false, ["deriveBits"]).then(key => window.crypto.subtle.deriveBits({
                name: "PBKDF2",
                salt: salt,
                iterations: iter,
                hash: hash
            }, key, keyLen * 8)).then(key => new Uint8Array(key)).catch(err => (err.name === "OperationError" || err.name === "NotSupportedError") ? (window.pbkdf2 = pbkdf2_js).apply(self, args) : (err.name === "InvalidAccessError") ? pbkdf2_js.apply(self, args) : Promise.reject(err));
        };
    } else {
        return function(password, salt, iter, keyLen, hash) {
            switch ((hash.name || hash).toUpperCase()) {
